...
func createUsersFromExcelHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, row := range rows {
		...
		// Try to create user with useradd (most Linux distributions)
		script.WriteString(fmt.Sprintf("echo '%s' | sudo -S useradd -m -s /bin/bash \"%s\"\n",
			server.RootPassword, linuxUsername))

		// Set password
		script.WriteString(fmt.Sprintf("echo '%s' | sudo -S bash -c \"echo '%s:%s' | chpasswd\"\n",
			server.RootPassword, linuxUsername, safePass))
